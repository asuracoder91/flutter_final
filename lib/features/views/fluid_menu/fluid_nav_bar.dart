...
class FluidNavBar extends ConsumerStatefulWidget {
  ...
  static const double nominalHeight = 68.0;
  final FluidNavBarChangeCallback? onChange;

  @override
  ConsumerState createState() => _FluidNavBarState();
}
...
class _FluidNavBarState extends ConsumerState<FluidNavBar>
  ...
  void triggerAnimationForIndex(int index) {
  ...
  Widget build(BuildContext context) {
    ...
    const height = FluidNavBar.nominalHeight; // Nav Bar 높이
  ...
  List<FluidNavBarButton> _buildButtons() {
    List<FluidFillIconData> icons = [
      FluidFillIcons.home,
      FluidFillIcons.fountainPen,
    ];
    var buttons = List<FluidNavBarButton>.filled(
        2, FluidNavBarButton(icons[0], false, () {})); // 버튼 갯수
    for (var i = 0; i < 2; ++i) {
...
class _BackgroundCurvePainter extends CustomPainter {
  static const _radiusTop = 50.0; // 위쪽 폭
  static const _radiusBottom = 50.0; // 아래쪽 폭
  ...
  static const _bottomY = 62.0; // 밑으로 파이는 정도
  ...
  _BackgroundCurvePainter(this._x, this._normalizedY, this._color);
...
class FluidNavBarController {
  late _FluidNavBarState _state;

  // ignore: library_private_types_in_public_api
  void attach(_FluidNavBarState state) {
    _state = state;
  }

  void triggerAnimationForIndex(int index) {
    _state._handlePressed(index);
  }
}

final fluidNavBarControllerProvider =
    Provider<FluidNavBarController>((ref) => FluidNavBarController());
